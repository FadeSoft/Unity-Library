...
public class RtsCameraMovement : MonoBehaviour
{
    ...
    private void CamMovement()
    {
        ...
        if (Input.GetKey(KeyCode.W) || Input.mousePosition.y >= Screen.height - rTSCamData.screenBorderPx) pos += transform.forward * speed * Time.deltaTime;
        if (Input.GetKey(KeyCode.S) || Input.mousePosition.y <= rTSCamData.screenBorderPx) pos += transform.forward * -1 * speed * Time.deltaTime;
        if (Input.GetKey(KeyCode.A) || Input.mousePosition.x <= rTSCamData.screenBorderPx) pos -= transform.right * speed * Time.deltaTime;
        if (Input.GetKey(KeyCode.D) || Input.mousePosition.x >= Screen.width - rTSCamData.screenBorderPx) pos += transform.right * speed * Time.deltaTime;
